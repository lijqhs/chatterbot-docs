...
- [ChatterBot用到的机器学习算法](#chatterbot用到的机器学习算法)
...
主进程在ChatterBot对象被初始化时会将用户配置的逻辑适配器到一个列表中，然后交MultiLogicAdapter 进行处理。
...
ChatterBot使用了不同的机器学习算法来生成对话的回复，主要涉及搜索算法和分类算法。

- 搜索算法

在ChatterBot取得回复的过程中，搜索算法用到了以下几个属性：
...
ChatterBot中的好几个逻辑适配器都使用了朴素贝叶斯分类算法来决定该逻辑适配器对某个输入语句生成的回复。
